...
// Map   is copy of sync.Map and use generic to prompt
...
// unchanged.
// 保障两个同时更新，时，后更新的那个更新成功，并返回前面那个的值。
...
// 1 避免删除过程中，新写入的值被删除。expunged 状态。可以让更新变为写到dirtymap。这样新值写入时如果时expunged 状态，则会加锁，等待dirty map 重建完成后
// 写入dirty map。
// 2 当状态变为expunge 时。更新key时，需要写入dirty map。 而不是更新原来的entry
...
type SizedMap[K, V comparable] struct {
	Map[K, V]
	length atomic.Int64
}

func (s *SizedMap[K, V]) Len() int {
	return int(s.length.Load())
}

func (s *SizedMap[K, V]) Store(key K, v V) {
	s.LoadOrStore(key, v)
}

func (s *SizedMap[K, V]) LoadOrStore(key K, v V) (old V, loaded bool) {
	old, loaded = s.Map.LoadOrStore(key, v)
	if !loaded {
		s.length.Add(1)
	}
	return
...
func (s *SizedMap[K, V]) Delete(key K) {
	s.LoadAndDelete(key)
}

func (s *SizedMap[K, V]) LoadAndDelete(key K) (v V, loaded bool) {
